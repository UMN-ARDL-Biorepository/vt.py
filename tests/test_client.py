...
def test_get_user(client):
    # This might fail if the user doesn't have permissions or ID is invalid
    # but we can try to get the current user info or a fixed ID
    # For now, we'll try with a dummy ID and catch HTTPError if expected
    try:
...
def test_gethistorydata(client):
    # This needs a valid object_id. We might be able to get one from getallmonitoredobjects
    objects_data = client.getallmonitoredobjects()
    # Assuming objects_data is a JSON string containing objects
    import json
...
def test_refresh_auth_token(client):
    # Only test if we have a token to refresh
...
def test_login_logout_manual():
    # Separate test for manual login/logout to verify token clearing
    api_url = os.getenv("API_URL")
    vt = VersaTrak(base_url=api_url)
